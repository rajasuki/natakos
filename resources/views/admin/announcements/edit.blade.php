...
@section('content')
    <div class="form-card">
        <form method="POST" action="{{ route('admin.announcements.update', $announcement) }}" id="announcement-form" enctype="multipart/form-data">
            @csrf @method('PUT')

            <div class="form-group">
                <label for="title" class="form-label">Judul</label>
                <input id="title" name="title" type="text" class="form-input" value="{{ old('title', $announcement->title) }}" required maxlength="200" oninput="refreshPreview()">
                @error('title') <span class="form-error">{{ $message }}</span> @enderror
            </div>

            <div class="form-group">
                <label for="content" class="form-label">Konten</label>
                <textarea id="content" name="content" class="form-input" rows="4" required oninput="refreshPreview()">{{ old('content', $announcement->content) }}</textarea>
            ...
            <div class="form-group">
                ...
                <div style="display:flex;gap:12px;align-items:center;">
                    <input id="scroll_speed" name="scroll_speed" type="range" class="form-input" style="flex:1;padding:0;border:none;" value="{{ old('scroll_speed', $announcement->scroll_speed ?? 180) }}" min="30" max="600" step="10" oninput="updateSpeedLabel()">
                    <span id="speed-label" style="font-size:14px;font-weight:600;color:var(--ui-ink);min-width:48px;text-align:right;">{{ $announcement->scroll_speed ?? 180 }}s</span>
                </div>
            ...
            <div class="form-group">
                ...
                <div class="sound-section">
                    <div class="field">
                        <label>
                            <input type="checkbox" name="has_sound" value="1" @checked(old('has_sound', $announcement->has_sound)) onchange="refreshPreview()">
                    ...
                    <div class="field">
                        ...
                        <select id="announcement_sound_id" name="announcement_sound_id" class="form-input" onchange="refreshPreview()">
                            <option value="">—</option>
                            @foreach ($sounds as $s)
                                <option value="{{ $s->id }}" data-url="{{ asset('storage/'.$s->file_path) }}" @selected(old('announcement_sound_id', $announcement->announcement_sound_id) == $s->id)>{{ $s->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    @if ($announcement->sound)
                        <div style="display:flex;align-items:center;gap:8px;padding:8px 12px;background:#fff;border-radius:var(--radius-md);border:1px solid var(--ui-border);">
                            <span class="material-symbols-outlined" style="font-size:18px;color:var(--ui-accent);">volume_up</span>
                            <span style="flex:1;font-size:13px;">Suara terpilih: <strong>{{ $announcement->sound->name }}</strong></span>
                            <audio controls preload="metadata" style="height:30px;width:140px;">
                                <source src="{{ asset('storage/'.$announcement->sound->file_path) }}">
                            </audio>
                        </div>
                    @endif
                </div>
            ...
            <div class="form-group">
                ...
                <div class="preview-banner">
                    <div class="preview-banner-head">
                        ...
                        <span id="preview-sound-badge" style="display:{{ ($announcement->has_sound && $announcement->announcement_sound_id) ? 'inline' : 'none' }};margin-left:auto;font-size:11px;color:var(--ui-accent);font-weight:600;">
                    ...
                    <div class="preview-banner-track">
                        ...
                        <div class="preview-bar">
                            <div class="preview-scroll" id="preview-scroll" style="animation-duration:{{ $announcement->scroll_speed ?? 180 }}s">
                                <span class="preview-item" id="preview-item-a">
                                    <span class="pi-badge">{{ $announcement->title }}</span>
                                    <span class="pi-dot"></span>
                                    <span class="pi-text">{{ $announcement->content }}</span>
                                </span>
...
<script>
    ...
    document.addEventListener('click', function(e) {
        if (e.target.id === 'preview-play-btn') {
            if (previewAudio) {
                previewAudio.pause();
                previewAudio = null;
                e.target.textContent = 'Dengar';
                return;
            }
            var url = null;
            var sel = document.getElementById('announcement_sound_id');
